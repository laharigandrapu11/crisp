#!/usr/bin/env bash
# Build and deploy stage1_ocr to Google Cloud Run.
#
...
REPO="${REPO:-crisp}"
SERVICE="${SERVICE:-stage1-ocr}"
IMAGE="${REGION}-docker.pkg.dev/${GCP_PROJECT_ID}/${REPO}/${SERVICE}"
TAG="${TAG:-$(date -u +%Y%m%d-%H%M%S)}"

# Resource sizing — Stage 1 has ~1.8MB of weights and runs CPU-only PyTorch.
# 2 CPU / 2 GiB is a comfortable baseline for a single concurrent request;
# bump CPU/memory if you need higher throughput per instance.
CPU="${CPU:-2}"
MEMORY="${MEMORY:-2Gi}"
